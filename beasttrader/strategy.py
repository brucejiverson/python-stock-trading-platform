...
from beasttrader.orders import MarketOrder, OrderBase, OrderSide
from beasttrader.indicators import IndicatorMapping
...
class StrategyBase(Base):
    ...
    def _get_cur_price_from_state(self, cur_state:Dict[str, float], ticker:str=None) -> float:
        """If ticker is none, returns for the first ticker"""
        if ticker is None:
...
class SimpleSignal(StrategyBase):
    ...
    def __init__(
        ...
        name ='beasttrader' + '.' + self.__class__.__name__ + '.' + str(account_number)[:8]
        super().__init__(account_number, name, tickers, indicator_mapping)
